<?php
class shopPddPluginCertificatesCollection 
{
	protected $hash;
	protected $model;
	protected $fields = [];
    ...
    {
		$this->hash = $hash;
		$this->model = new shopPddPluginCertificatesModel();
	...
	public function workupItems(&$items = array())
    {
		...
		$base_url = wa()->getDataUrl('plugins/pdd/Certificates/', true, 'shop', false);

		foreach($items as $key => $item) 
		{
			$file_url = $item['product_id'].'/'. $item['id'].'.'.$item['ext'];
			$items[$key]['download_url'] = $base_url.$file_url;
		}
	}
